...
#	name=$(ls -F /var/ftp|grep '/$'|sed '/upload/d')
	webdavrw=$(uci get sysmonitor.sysmonitor.webdav_rw)
	...
	ftprw=$(uci get sysmonitor.sysmonitor.ftp_rw)
	if [ "$ftprw" == 0 ]; then
...
<head>
<meta http-equiv="refresh" content="0;url=http://$hostname:8080"> 
</head>
EOF
	ip=$(ip -o -4 addr list br-lan | cut -d ' ' -f7|cut -d'/' -f1)
	...
	if [ -n "$ip6" ]; then
		...
		[ "$(uci_get_by_name $NAME $NAME samba 0)" == 1 ] && /etc/init.d/samba4 restart
	else
		ip6=$ip
		echo "" > /www/ip6.html
	fi
...
minidlna_chk() {
	str=$(uci_get_by_name $NAME $NAME minidlna_dir 0)','
...
check_dir() {
	str=$(uci_get_by_name $NAME $NAME $1 0)','
	str=$(echo $str|sed 's/,,/,/g')
	num=$(echo $str|awk -F"," '{print NF-1}')
	a=1
	while [ $a -le $num ]
...
[ -f /etc/config/minidlna ] && sed -i '/media_dir/d' /etc/config/minidlna
[ -f /tmp/NAS.OK ] && rm /tmp/NAS.OK
for m in $syssd
do
...
EOF
	...
	[ "$(uci_get_by_name $NAME $NAME minidlna 0)" == 1 ] && {
	...
	[ -f /etc/config/minidlna ] && uci commit minidlna
	[ "$(uci_get_by_name $NAME $NAME ftp 0)" == 1 ] && /etc/init.d/vsftpd start	
	[ "$(uci_get_by_name $NAME $NAME nfs 0)" == 1 ] && /etc/init.d/nfs start
	[ "$(uci_get_by_name $NAME $NAME samba 0)" == 1 ] && /etc/init.d/samba4 start
	[ "$(uci_get_by_name $NAME $NAME minidlna 0)" == 1 ] && /etc/init.d/minidlna start
	rm /tmp/music
	touch /tmp/NAS.OK
}
...
sysupgrade() {
	...
	if [ -n "$file" ]; then
		echo '------------------------------------------------------------------------------------------------------' > $SYSLOG
		sysupgrade='/usr/sbin/openwrt-update-rockchip /tmp/upload/'$file' no no-restore'
...
sysbutton() {
	...
	#		button=$button' <button class="button1" title="Close VPN"><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=CloseVPN&sys1=&redir=system">CloseVPN</a></button>'
		...
		;;
	nas)
		button='<button class="button1" title="NAS restart"><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=NASrestart&sys1=&redir=nas">NAS(restart)</a></button>'
		if [ -f /etc/init.d/lighttpd ]; then
			if [ ! -n "$(pgrep -f lighttpd)" ]; then
				color='class=button2 title="WebDAV is not ready...,Restart"'
			else
				color='class=button1 title="WebDAV restart"'
			fi
			button=$button' <button '$color'><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=NASwebdav&sys1=&redir=nas">WebDAV</a></button>'
		fi
		if [ -f /etc/init.d/vsftpd ]; then
			if [ ! -n "$(pgrep -f vsftpd)" ]; then
				button=$button' <button class=button2 title="VSftpd is not ready...,Restart"><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=NASvsftp&sys1=&redir=nas">VSFTP</a></button>'
			else
				button=$button' <button class="button1" title="Goto VSftps setting"><a href="/cgi-bin/luci/admin/services/vsftpd" target="blank">VSFTP</a></button>'
			fi
		fi
		if [ -f /etc/init.d/samba4 ]; then
			if [ ! -n "$(pgrep -f smbd)" ]; then
				button=$button' <button class=button2 title="Samba is not ready...,Restart"><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=NASsamba&sys1=&redir=nas">SAMBA</a></button>'
			else
				button=$button' <button class=button1 title="Goto samba setting"><a href="/cgi-bin/luci/admin/services/samba4" target="blank">SAMBA</a></button>'
			fi
		fi
		if [ -f /etc/init.d/nfsd ]; then
			if [ ! -n "$(pgrep -f nfsd)" ]; then
				button=$button' <button class=button2 title="NFS is not ready...,Restart"><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=NASnfs&sys1=&redir=nas">NFS</a></button>'
			else
				button=$button' <button class=button1 title="Goto NFS setting"><a href="/cgi-bin/luci/admin/services/nfs" target="blank">NFS</a></button>'
			fi
		fi
		if [ -f /etc/init.d/minidlna ]; then
			if [ ! -n "$(pgrep -f minidlnad)" ]; then
				button=$button' <button class=button2 title=""Minidlna is not ready...,Restart><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=NASminidlna&sys1=&redir=nas">MiniDLNA</a></button>'
			else
				button=$button' <button class=button1 title="Goto minidlna setting"><a href="/cgi-bin/luci/admin/services/minidlna" target="blank">MiniDLNA</a></button>'
			fi
		fi
		;;
	nastitle)
		...
		button=' <button class="button1" title="Start terminal"><a href="/cgi-bin/luci/admin/services/ttyd" target="_blank">Terminal</a></button>'
	#	button=$button' <button class="button1"><a href="/cgi-bin/luci/admin/sys/sysmonitor/sysmenu?sys=UpdateDDNS&sys1=&redir='$redir'">UpdateDDNS</a></button>'
		;;
	buttontitle)
		redir='system'
		[ "$(uci get sysmonitor.sysmonitor.ddnslog)" == 1 ] && redir='log'
